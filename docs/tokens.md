...
| `--accent-text` | 文字・アイコンに使うアクセント（リンク・選択中のタブ等）。`--accent` から明度だけを制限して導き、light は 0.48 以下・dark は 0.68 以上にして背景と 4.5:1 以上を保つ。相対色構文に非対応のブラウザでは `--accent` と同じ。**accent を文字色に使うときは `--accent` でなくこれを使う**（`--accent` は主ボタンの地にも使うため明度を動かせない） |

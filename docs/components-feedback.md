...
- **タイトルの見出しレベルはページ内の文脈に合わせる**（固定ではない）。装飾は `> section > header > :first-child` でレベル非依存に当たるため、見た目を変えずにレベルだけ調整できる。例: ページ直下（`h1` 直後）のモーダルは `h2`、`h2` セクション配下のモーダルは `h3`。レベルのスキップを作らないこと
